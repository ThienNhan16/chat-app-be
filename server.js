...
io.on("connection", async (socket) => {
  ...
  socket.on("start_audio_call", async (data, socket) => {
    ...
    try {
      if (to_user.socket_id) {
        ...
        console.log("audio_call_notification Successful");
  ...
  socket.on("audio_call_accepted", async (data) => {
    ...
    try{
      AudioCall.findOneAndUpdate(
        {
          participants: { $size: 2, $all: [!to ? to_instead : null, from] },
        },
        { verdict: "Accepted" },
        {new: true},
        (err, updatedAudio) => {
          ...
          console.log("updatedAudio", updatedAudio);
          // console.log("fromuser?._id", from_user?._id);
